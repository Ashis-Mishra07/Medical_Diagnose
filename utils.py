...
def extract_headlines(cleaned_text: str) -> str:
    ...
    for line in lines:
        if line == "More":
            if current_block:
                headlines.append(" ".join(current_block))
                current_block = []
        else:
            if len(line) > 20 and len(line) < 200:  # Likely headline length
    ...
    return "\n".join(headlines[:10])  # Return top 10 headlines
...
def generate_broadcast_news(api_key, news_data, reddit_data, topics):
    """Generate broadcast news from news and reddit data"""
    system_prompt = """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Broadcast generation error: {str(e)}")
...
def tts_to_audio(text: str, language: str = "en") -> str:
    ...
    try:
        ...
        audio_path = os.path.join(temp_dir, f"news_audio_{datetime.now().strftime('%Y%m%d_%H%M%S')}.mp3")
...
def summarize_with_ollama(headlines) -> str:
    """Fallback function using Ollama for summarization"""
    try:
        response = ollama.chat(model='llama2', messages=[
            {
                'role': 'user',
                'content': f'Summarize these news headlines into a brief news report: {headlines}'
            }
        ])
